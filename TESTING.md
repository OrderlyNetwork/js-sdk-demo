...
| T-039 | Release tag | 分别为 demo、dmm 执行 `pnpm release:dev:<target> -- --dry-run`、`pnpm release:qa:<target> -- --dry-run`、`pnpm release:app:<target> -- --dry-run`，并在 `main` 执行 `pnpm release:prod:<target> -- --dry-run` | `orderly-release-tag` 根据远端稳定 tag 计算对应 target 的环境 tag，且不读取 `PACKAGE_VERSION` |
