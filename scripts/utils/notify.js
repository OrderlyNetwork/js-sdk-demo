...
// Keep best-effort notifications from blocking release failures indefinitely.
...
function formatTelegramMessage(message, link) {
  const linkLabel = link
    ? Array.from(link.label)
        .slice(0, TELEGRAM_MAX_MESSAGE_LENGTH - 1)
        .join("")
    : "";
  const linkLength = link ? Array.from(linkLabel).length + 1 : 0;
  const truncatedMessage = Array.from(message)
    .slice(0, Math.max(0, TELEGRAM_MAX_MESSAGE_LENGTH - linkLength))
    .join("");
  ...
  return `${formattedMessage}\n<a href="${escapeHtml(link.url)}">${escapeHtml(linkLabel)}</a>`;
}
...
async function fetchWithTimeout(url, options, provider, secrets) {
  ...
  try {
    ...
    if (controller.signal.aborted) {
      throw new Error(
        `${provider} request timed out after ${NOTIFICATION_REQUEST_TIMEOUT_MS}ms`,
      );
    }

    throw createRequestError(provider, error, secrets);
  } finally {
